...
qint32 QMqttClient::publish(const QMqttTopicName &topic, const QByteArray &message, quint8 qos, bool retain)
{
    ...
    if(d->m_attackType !="MQTT Publish Flood")
    {
        d->m_publish_packet_per_sec = 50;
        for(int k=0;k<d->m_publish_packet_per_sec;k++)
           d->m_connection.sendControlPublish(topic, message, qos, retain);
    }
...
QString QMqttClient::attackType() const //Added by Faisal
{
    Q_D(const QMqttClient);
    return d->m_attackType;
}
...
void QMqttClient::connectToHost(bool encrypted, const QString &sslPeerName)
{
    ...
    if (!d->m_connection.ensureTransportOpen(sslPeerName)) //'attackType' Added by Faisal
    {
...
void QMqttClient::setAttackType(const QString &attackType) //Added by Faisal
{
    Q_D(QMqttClient);
    if (d->m_attackType == attackType)
        return;

    d->m_attackType = attackType;
    emit attackTypeChanged(attackType);
}
